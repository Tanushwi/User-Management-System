...
// In-memory pubsub using EventEmitter
const pubsub = new EventEmitter();

// simple in-memory users store (demo). Replace with DB later.
...
// share pubsub & store with routes via locals
...
io.on('connection', (socket) => {
  ...
  // send existing online count
  socket.emit('welcome', { msg: 'Connected', socketId: socket.id });

  // forward pubsub broadcasts to all sockets
  const handler = (data) => {
...
// helper publish function used by routes/controllers
